...
def render(manifest: dict, segment: dict, repo_root: Path) -> Image.Image:
    ...
    if layout == "split_2up" and len(selected) != 2:
        raise SystemExit("split_2up preview requires exactly two selected assets")
    ...
    if layout == "split_2up":
        ...
        for index, item in enumerate(selected):
            ...
            if fit_mode == "contain":
                panel = ImageOps.contain(source, (panel_width, content_height), method=Image.Resampling.LANCZOS)
                panel_canvas = Image.new("RGB", (panel_width, content_height), (245, 245, 248))
                panel_canvas.paste(panel, ((panel_width - panel.width) // 2, (content_height - panel.height) // 2))
                panel = panel_canvas
            else:
